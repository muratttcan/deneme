// Sweep over freedChainWeight (hand=5 only): does rewarding face-down
// reveals that could unblock tentatively-unsafe 1-4 chain-catchables
// add gold rate?
//
// Usage: node ctk/bench-freed.mjs [games] [seed] [csv-of-weights]
...
function withFreedW(value) {
  return {
    ...DEFAULT_WEIGHTS,
    freedChainWeight: { early: 0, mid: 0, late: value },
  };
}

function main() {
  const N = Number(process.argv[2] ?? 6000);
  const seed = Number(process.argv[3] ?? 0xFEED);
  const sweepArg = process.argv[4];
  const sweep = sweepArg ? sweepArg.split(",").map(Number) : [0, 0.5, 1, 1.5, 2, 3, 5];
  const boards = generateBoards(N, seed);
  console.log(`freedChainWeight sweep — paired self-play, n=${N}, seed=0x${seed.toString(16)}`);
  console.log(`  w=0 = current shipped baseline (no free-the-prisoner bonus).`);
  console.log("");
  const results = [];
  const t0 = Date.now();
  for (let i = 0; i < sweep.length; i++) {
    const w = sweep[i];
    const r = evaluate(withFreedW(w), boards);
    results.push({ w, ...r });

// 1D sweep over plantPenaltyWeight (early/mid only; late stays 0 because
// hand=5 already has its own catchPenalty). Paired self-play vs current
// shipped DEFAULT_WEIGHTS.
//
// Usage: node ctk/bench-plant.mjs [games] [seed] [csv-of-weights]
...
function withPlantW(value) {
  return {
    ...DEFAULT_WEIGHTS,
    plantPenaltyWeight: { early: value, mid: value, late: 0 },
  };
}
...
function main() {
  ...
  const sweep = arg ? arg.split(",").map(Number) : [0, 0.25, 0.5, 0.75, 1.0, 1.5, 2.0];
  const boards = generateBoards(N, seed);
  console.log(`plantPenaltyWeight sweep — paired self-play, n=${N}, seed=0x${seed.toString(16)}`);
  console.log(`  w=0 = current shipped baseline (no plant penalty).`);
  console.log("");
  const results = [];
  const t0 = Date.now();
  for (let i = 0; i < sweep.length; i++) {
    const w = sweep[i];
    const r = evaluate(withPlantW(w), boards);
    results.push({ w, ...r });
